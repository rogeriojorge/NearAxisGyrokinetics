program test_vmec_to_gs2_geometry_interface
  ...
  character(len=2000) :: vmec_filename, gs2_output, gx_output, geometry_output,number_of_field_periods_to_includestr, desired_normalized_toroidal_fluxstr, nzgridstr, nlambdastr
  integer, parameter :: nalpha = 1
  !integer, parameter :: nzgrid = 350
  !integer, parameter :: nlambda = 40
  integer :: nzgrid, nlambda
  ...
  real, allocatable, dimension(:, :) :: bmag, gradpar, gds2, gds21, gds22, gbdrift, gbdrift0, cvdrift, cvdrift0, grho
  ...
  call get_command_argument(7,nzgridstr)
  read(nzgridstr , *) nzgrid
  call get_command_argument(8,nlambdastr)
  read(nlambdastr , *) nlambda

  allocate (zeta(-nzgrid:nzgrid))
  allocate (bmag(nalpha,-nzgrid:nzgrid),gradpar(nalpha,-nzgrid:nzgrid),gds2(nalpha,-nzgrid:nzgrid),gds21(nalpha,-nzgrid:nzgrid),gds22(nalpha,-nzgrid:nzgrid),gbdrift(nalpha,-nzgrid:nzgrid),gbdrift0(nalpha,-nzgrid:nzgrid),cvdrift(nalpha,-nzgrid:nzgrid),cvdrift0(nalpha,-nzgrid:nzgrid),grho(nalpha,-nzgrid:nzgrid))
  allocate (lambda(nalpha,nlambda))
  ...
  write (iunit,*) nzgrid, 1, 2*nzgrid, 1., 1., shat, 1, safety_factor_q
  ...
  write (iunit,*) nzgrid, 1, 2*nzgrid, 1., 1., shat, 1, safety_factor_q, 1.0
